<template>
  <div class="relative" data-testid="newsletter-box">
    <picture v-if="backgroundImage">
      <source :srcset="backgroundImage.desktop" media="(min-width: 768px)" />
      <img :src="backgroundImage.mobile" class="absolute w-full h-full z-[-1] md:object-cover" />
    </picture>
    <div class="p-4 sm:p-10 text-center" :style="{ 'background-color': backgroundColor }">
      <p class="typography-headline-4 sm:typography-headline-3 font-semibold">
        {{ header }}
      </p>
      ...
      <form
        class="mb-4 flex flex-col md:flex-row gap-4 max-w-[688px] mx-auto"
      ...
      >
        ...
        <SfButton type="submit" size="lg"> {{ buttonText }} </SfButton>
      </form>
...
type NewsletterBoxProps = AgnosticCmsNewsletterBoxProps;

defineProps<NewsletterBoxProps>();
