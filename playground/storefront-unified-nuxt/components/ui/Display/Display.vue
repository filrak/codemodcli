      ...
      >
        ...
        >
          <div class="flex flex-1 flex-col justify-center items-center md:items-start p-6 lg:p-10 max-w-1/2">
            <p :class="['uppercase typography-text-xs block font-semibold tracking-widest', item.subtitleClass]">
              {{ item.subtitle }}
            </p>
            <h2 :class="['mb-4 mt-2 font-semibold typography-headline-3', item.titleClass]">
              {{ item.title }}
            </h2>
            <p class="typography-text-base block text-center md:text-left mb-4">{{ item.description }}</p>
            <SfButton class="!bg-black" :tag="NuxtLinkLocale" :to="paths.category">
              {{ item.buttonText }}
            </SfButton>
          </div>
          <NuxtImg
