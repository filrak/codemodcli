<template>
  <UiOverlay visible>
    ...
    >
      <header
        class="flex justify-between bg-white items-center typography-headline-4 md:typography-headline-3 font-semibold"
      >
      ...
      <main class="mt-6">
        <ul class="bg-neutral-100 p-4 rounded-md md:columns-2 mb-6">
          ...
          <li class="my-4 md:mb-0">
            ...
            <span>{{ data?.orderDate ? new Date(data?.orderDate).toLocaleDateString() : '-' }}</span>
          </li>
          <li>
            <p class="font-medium">{{ $t('account.ordersAndReturns.orderDetails.paymentAmount') }}</p>
            <span>${{ data?.totalPrice.precisionAmount }}</span>
          </li>
          <li class="mt-4">
        ...
        <div v-for="(product, i) in data?.lineItems" :key="i" class="md:hidden border-t border-neutral-200">
          ...
          <ul class="flex">
            <li class="flex-grow pr-4 py-4">
              ...
              <span>${{ product.unitPrice?.precisionAmount }}</span>
            </li>
            ...
            <li class="flex-[50%] justify-self-end text-right pl-4 py-4">
              ...
              <span>${{ product.totalPrice?.precisionAmount }}</span>
            </li>
          </ul>
        </div>
        <table class="hidden md:table w-full text-left typography-text-sm mx-4 md:mx-0">
          ...
          <tbody>
            <tr v-for="(product, i) in data?.lineItems" :key="i" class="border-b border-neutral-200 align-top">
              <td class="pb-4 pr-4 typography-text-base">
                <UiProductCardHorizontal :product="product" />
              </td>
              <td class="p-4 lg:whitespace-nowrap typography-text-base">${{ product.unitPrice?.precisionAmount }}</td>
              <td class="p-4 typography-text-base">{{ product.quantity }}</td>
              <td class="p-4 typography-text-base">${{ product.totalPrice?.precisionAmount }}</td>
            </tr>
          </tbody>
        </table>
        ...
        <ul class="md:columns-2 mt-6">
          ...
          <li class="mb-4">
            ...
            <UiAddress v-if="data?.shippingAddress" :address="data?.shippingAddress" class="not-italic" />
          </li>
          <li class="mb-4 md:mb-0">
            <p class="typography-text-sm font-medium mb-2">
              {{ $t('account.ordersAndReturns.orderDetails.paymentMethod') }}
...
watch(
  isOpen,
  (isOpen) => {
    if (!isOpen) router.push(localePath(paths.accountMyOrders));
  },
  { immediate: true },
);
